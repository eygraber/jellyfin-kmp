...
# Usage: ./remove-blocking.sh BLOCKED_ISSUE BLOCKING_ISSUE
#
# Arguments:
#   BLOCKED_ISSUE    Issue number that was blocked
#   BLOCKING_ISSUE   Issue number that was doing the blocking
#
...
if [[ -z "$1" ]] || [[ -z "$2" ]]; then
  ...
  echo "Example: $0 123 45  # Remove: issue 123 blocked by issue 45" >&2
  exit 1
fi
...
echo "Removed: #$BLOCKED_NUMBER is no longer blocked by #$BLOCKING_NUMBER"

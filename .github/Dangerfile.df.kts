...
danger(args) {
  with(github) {
    ...
    #--------------------------------------------------------------------------------------------------------------------
    */
    val labelsToFilter = setOf("WIP", "hold", "skip release notes")
    val acceptableLabels = labelNames.filter { it !in labelsToFilter }

    if(acceptableLabels.isEmpty()) {
      fail("PR needs labels (WIP, hold, and skip release notes don't count)")
    }
    ...
    # --------------------------------------------------------------------------------------------------------------------
    */
    if("WIP" in labelNames) fail("This PR cannot be merged with a WIP label applied")
  ...
  /*
  # --------------------------------------------------------------------------------------------------------------------
  # Make sure that any PR update commits are squashed
  # --------------------------------------------------------------------------------------------------------------------
  */
  val temporaryCommitPrefixes = listOf("pr update", "wip", "todo", "fixme", "***")
  if(git.commits.any { commit ->
      temporaryCommitPrefixes.any { prefix ->
        commit.message.startsWith(prefix, ignoreCase = true)
      }
    }) {
    fail("Please squash any temporary commits")
  }

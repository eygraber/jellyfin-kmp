...
class VideoPlayerModel(
  ...
  /**
   * Releases player resources without contacting the server.
   *
   * Used as a synchronous cleanup hook (e.g. on `DisposableEffect.onDispose`) so the underlying
   * player is always torn down even when the user navigates away through a path that bypasses
   * [stop] (such as system back / gesture back).
   */
  fun releasePlayer() {
    playerService.release()
  }
